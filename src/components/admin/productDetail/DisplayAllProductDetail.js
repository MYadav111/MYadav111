...
export default function DisplayAllProductDetail() {
      ...
      <Grid container spacing={2}>
        ...
        <Grid item xs={4}>
          <FormControl fullWidth>
            ...
            <Select value={productStatus} error={errorMessages?.productStatus} onFocus={() => handleErrorMessages("productStatus", null)} label="Product Status" onChange={(e) => setProductStatus(e.target.value)}>
        ...
        <ReactQuill
               ...
               formats={["header","font","size","bold","italic","underline","strike", "blockquote","list","bullet","indent","link","image","video","code-block"]} theme="snow" value={productDetailDescription} onChange={setProductDetailDescription} />
        </Grid>
  ...
  const fetchAllProdcutDetail = async () => {
    ...
    if (result.status) {
      setProducDetailList(result.data);
    } else {
