...
export default function Otp() {
  ...
  const handleVerify = async () => {
    if (otp == genOtp) {
      var response = await postData("userinterface/check_user_mobileno", {
        mobileno,
      });
      if (response.status) {
        dispatch({
        ...
        var res = await postData("userinterface/check_user_address", {
      ...
      >
        ...
        >
          <div style={{ padding: 15 }}>
            <div>
              <ClearIcon style={{ width: 20, height: 20, cursor: "pointer" }}/>
            </div>
            ...
            >
              Enter the OTP sent to you on<br></br>
